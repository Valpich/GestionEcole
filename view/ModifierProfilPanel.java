 ...
 */
public class ModifierProfilPanel extends JPanel implements ActionListener {
	...
	/**
	 * Accesseur en lecture sur la personne.
	 * 
	 * @return La personne.
	 */
	...
	/**
	 * Accesseur en ecriture sur la personne.
	 * 
	 * @param personne
	...
	/**
	 * Accesseur en lecture sur l'adresse.
	 * 
	 * @return L'adresse.
	 */
	...
	/**
	 * Accesseur en ecriture sur l'adresse.
	 * 
	 * @param adresse
	 *            La nouvelle adresse.
	 */
	...
	public ModifierProfilPanel() {
		...
		JLabel lblModifierProfil = new JLabel("Modifier le profil");
		lblModifierProfil.setFont(new Font("Tahoma", Font.BOLD, 20));
		add(lblModifierProfil, "4, 2, 15, 1, center, default");
		...
		add(btnModifierProfil, "16, 28, 3, 1");
	...
	/**
	 * Construit le panneau de modification du profil d'une personne.
	 * 
	 * @param application
	 *            L'application du panneau.
	 */
	public ModifierProfilPanel(final Application application) {
		this();
	 ...
	 *            L'id de la personne.
	 */
	public ModifierProfilPanel(final Application application, Integer idPersonne) {
		this();
		this.setApplication(application);
		this.setIdPersonne(idPersonne);

		actualiserAcces();
		actualiserTextFieldsIdPersonne(idPersonne);
	}

	/**
	 * Permet de changer en fonctionne de l'acces.
	 */
	private void actualiserAcces() {
		if (getApplication().getGestionnaireModele().getNiveauAcces() == 1
				|| getApplication().getGestionnaireModele().getNiveauAcces() == 2) {
			tfNom.setEditable(false);
			tfPrenom.setEditable(false);
			tfJour.setEditable(false);
			tfMois.setEditable(false);
			tfAnnee.setEditable(false);
			tfPromo.setEditable(false);
			tfAnneeArrivee.setEditable(false);
			tfUsername.setEditable(false);
			chckbxRedoublant.setEnabled(false);
			cbTypePersonne.setEnabled(false);

		} else if (getApplication().getGestionnaireModele().getNiveauAcces() == 0) {
			tfNom.setEditable(true);
			tfPrenom.setEditable(true);
			tfAdresse.setEditable(true);
			tfJour.setEditable(true);
			tfMois.setEditable(true);
			tfAnnee.setEditable(true);
			tfPromo.setEditable(true);
			tfAnneeArrivee.setEditable(true);
			tfUsername.setEditable(true);
			chckbxRedoublant.setEnabled(true);
			cbTypePersonne.setEditable(false);
	...
	/**
	 * Permet d'actualiser des champs.
	 */
	@SuppressWarnings("deprecation")
	private void actualiserTextFieldsIdPersonne(Integer idPersonne) {
		if (idPersonne == null)
		...
		for (Administrateur administrateur : getApplication().getGestionnaireModele().getAdministrateurs())
			if (administrateur.getId().equals(idPersonne))
				setPersonne(administrateur);

		for (Professeur professeur : getApplication().getGestionnaireModele().getProfesseurs())
			if (professeur.getId().equals(idPersonne))
				setPersonne(professeur);

		for (Etudiant etudiant : getApplication().getGestionnaireModele().getEtudiants())
			if (etudiant.getId().equals(idPersonne))
				setPersonne(etudiant);

		if (getPersonne() != null) {
			for (Adresse adresses : getApplication().getGestionnaireModele().getAdresses())
				if (adresses.getId().equals(getPersonne().getIdAdresse()))
					setAdresse(adresses);

			DecimalFormat formatMoisJour = new DecimalFormat("00");
			DecimalFormat telephone = new DecimalFormat("0000000000");

			tfNom.setText(getPersonne().getNom());
			tfPrenom.setText(getPersonne().getPrenom());
			tfNumero.setText("");
			tfAdresse.setText("");
			tfVille.setText("");
			tfCodePostal.setText("");
			tfPays.setText("");
			tfJour.setText(formatMoisJour.format(getPersonne().getDateDeNaissance().getDate()));
			tfMois.setText(formatMoisJour.format(getPersonne().getDateDeNaissance().getMonth() + 1));
			tfAnnee.setText(Integer.toString(getPersonne().getDateDeNaissance().getYear() + 1900));
			tfMailPerso.setText(getPersonne().getEmailPersonnel());
			tfMailPro.setText(getPersonne().getEmailProfessionnel());
			tfTelPerso.setText(telephone.format(getPersonne().getTelephoneMobile()));
			tfTelPro.setText(telephone.format(getPersonne().getTelephoneFixe()));
			tfPromo.setText(getPersonne().getPromotion());
			tfAnneeArrivee.setText(Integer.toString(getPersonne().getAnneeArrivee().getYear() + 1900));
			tfUsername.setText(getPersonne().getIdentifiant());
			if (adresse != null) {
				tfNumero.setText(Integer.toString(getAdresse().getNumero()));
				tfAdresse.setText(getAdresse().getRue());
				tfVille.setText(getAdresse().getVille());
				tfCodePostal.setText(Integer.toString(getAdresse().getCodePostal()));
				tfPays.setText(getAdresse().getPays());
				if (getAdresse().getNomAdresse().equals("Adresse principale"))
					cbIntituleAdresse.setSelectedIndex(0);
				else
					cbIntituleAdresse.setSelectedIndex(1);

				if (getPersonne().isRedoublant())
					chckbxRedoublant.setSelected(true);
				else
					chckbxRedoublant.setSelected(false);

				if (getPersonne().getPromotion().equals("Administrateur"))
					cbTypePersonne.setSelectedIndex(2);
				else if (getPersonne().getPromotion().equals("Professeur"))
					cbTypePersonne.setSelectedIndex(1);
				else
					cbTypePersonne.setSelectedIndex(0);
			}
		}
	...
	/**
	 * Permet d'actualiser les champs.
	 */
	private void actualiserTextFields() {
		actualiserTextFieldsIdPersonne(null);
	}
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (arg0.getSource() == btnModifierProfil) {
			...
			if (erreur.length() > 3)
				...
				if (getAdresse() != null) {
					Adresse adresseModifiee = getAdresse();
					adresseModifiee.setNumero(Integer.parseInt(tfNumero.getText()));
					adresseModifiee.setRue(tfAdresse.getText());
					adresseModifiee.setCodePostal(Integer.parseInt(tfCodePostal.getText()));
					adresseModifiee.setVille(tfVille.getText());
					adresseModifiee.setPays(tfPays.getText());
					adresseModifiee.setNomAdresse(cbIntituleAdresse.getSelectedItem().toString());
					getApplication().getGestionnaireModele().getGestionnaireSQL().altererAdresse(adresseModifiee);
				}

				getApplication().getGestionnaireModele().miseAJourListes();

				if (getPersonne() != null) {
					Personne personneModifiee = getPersonne();
					personneModifiee.setNom(tfNom.getText());
					personneModifiee.setPrenom(tfPrenom.getText());
					personneModifiee.setDateDeNaissance(Application.creerDate(Integer.parseInt(tfAnnee.getText()),
							Integer.parseInt(tfMois.getText()) - 1, Integer.parseInt(tfJour.getText())));
					personneModifiee.setEmailPersonnel(tfMailPerso.getText());
					personneModifiee.setEmailProfessionel(tfMailPro.getText());
					personneModifiee.setTelephoneMobile(Integer.parseInt(tfTelPerso.getText()));
					personneModifiee.setTelephoneFixe(Integer.parseInt(tfTelPro.getText()));
					personneModifiee.setRedoublant(chckbxRedoublant.isSelected());
					personneModifiee
							.setAnneeArrivee(Application.creerDate(Integer.parseInt(tfAnneeArrivee.getText()), 0, 1));
					personneModifiee.setIdentifiant(tfUsername.getText());

					if (cbTypePersonne.getSelectedIndex() == 2) {
						personneModifiee.setPromotion("Administrateur");
					} else if (cbTypePersonne.getSelectedIndex() == 1) {
						personneModifiee.setPromotion("Professeur");
					} else
						personneModifiee.setPromotion(tfPromo.getText());

					getApplication().getGestionnaireModele().getGestionnaireSQL().altererPersonne(personneModifiee);
					getApplication().getGestionnaireModele().miseAJourListes();
				}
				JOptionPane.showMessageDialog(null,
						tfPrenom.getText() + " " + tfNom.getText() + " a bien \u00E9t\u00E9 modifi\u00E9",
